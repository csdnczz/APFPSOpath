...
rastrigin = 20 + pos[0]**2 + pos[1]**2 - 10*np.cos(2*np.pi*pos[0]) -
10*np.cos(2*np.pi*pos[1])
return rastrigin
...
# 0.02*(g_value/(particle.b_value + g_value))*(particle.b_position -
particle.position[-1]) + \
# 0.1*(particle.b_value/(particle.b_value + g_value))*(g_position -
particle.position[-1])
return new_velocity
class Particle:
...
self.velocity = 0.01*np.array([-1 + 2*np.random.random(), -1 +
2*np.random.random()])
...
line3.set_data(np.array([0, 500]), np.array([g_position[1],
g_position[0]*500+g_position[1]]))
...
simulation = FuncAnimation(fig, update, blit=False, frames=2000, interval=100,
repeat=False)
